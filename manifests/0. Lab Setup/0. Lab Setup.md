...
*If not deploying Online Boutique to GCP, there is a metadata lookup that fails and causes extremely slow page loads. A custom install manifest has been included that will deploy the application without these lookups enabled.*
...
```bash
kubectl run multitool -n hipstershop --image=praqma/network-multitool
```
